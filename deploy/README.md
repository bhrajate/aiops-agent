...
| 服务 | 端口 | 用途 |
|---|---|---|
| PostgreSQL + pgvector | 5432 | 业务库(Incident/Investigation/Evidence/知识向量),事实源。schema 由 `make migrate` 建立 |
...
- 业务库 schema 由 `make migrate`(即 `control-plane migrate up`)建立,与生产同一条路径。
  新增迁移后重跑 `make migrate` 即可,**不需要**重建数据卷。`make seed` 灌开发种子(幂等)。
