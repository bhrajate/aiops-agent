...
# 2. 启动 Go 控制面(API + Ingress + Incident Manager + Tool Gateway)
cd ../control-plane && make run

# 3. 启动 Cluster Agent(只读工具服务)
cd ../cluster-agent && make run

# 4. 启动 Python RCA Worker
cd ../ai-worker && make run

# 5. 启动前端 Workbench
cd ../frontend && npm install && npm run dev
```

详见各子目录的 README 与 [`docs/RUNBOOK.md`](docs/RUNBOOK.md)。
